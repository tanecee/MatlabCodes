...
if start_idx < 1 || start_idx + p.wformLength - 1 > length(rx_data)
    warning('Senkronizasyon indeksi geçersiz, veriyi başlangıçtan itibaren kullanıyorum.');
    start_idx = 1; % Başlangıçtan itibaren al
    rx_data = rx_data(1:p.wformLength); % İlk wformLength kadar veri al
else
    rx_data = rx_data(start_idx:start_idx + p.wformLength - 1);
end

% Residual CFO estimation using CP
